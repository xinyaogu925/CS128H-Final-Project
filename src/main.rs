...
#[command(
    author,
    version,
    about = "EchoSafe - Audio Slicer, Spectral Processor & Security Suite"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    Process(ProcessArgs),
    Encrypt(FileCryptoArgs),
    Decrypt(FileCryptoArgs),
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Process(args) => process_audio(args),
        Commands::Encrypt(mut args) => {
            crypto::encrypt_file_to_path(&args.input, &args.output, &mut args.password)?;
            println!("Encrypted {} -> {}", args.input, args.output);
            Ok(())
        }
        Commands::Decrypt(mut args) => {
            crypto::decrypt_file_to_path(&args.input, &args.output, &mut args.password)?;
...
fn process_audio(args: ProcessArgs) -> Result<()> {
    // --- 1. Audio Decoding ---
    ...
    let mut end_idx =
        (args.end.unwrap_or(all_samples.len() as f32 / sample_rate_f) * sample_rate_f) as usize;
...
fn decode_audio(input_path: &str) -> Result<(Vec<f32>, usize)> {
    ...
    } else if input_path.ends_with(".m4a") || input_path.ends_with(".mp4") {
...
fn apply_highpass_filter(samples: &[f32], sample_rate: usize, cutoff_hz: f32) -> Vec<f32> {
    ...
    samples
        .iter()
        .zip(lowpassed.iter())
        .map(|(orig, low)| (orig - low).clamp(-1.0, 1.0))
        .collect()
}
...
fn apply_notch_filter(
    ...
    for i in 2..samples.len() {
        out[i] =
            samples[i] + b1 * samples[i - 1] + samples[i - 2] - a1 * out[i - 1] - a2 * out[i - 2];
    }
    out
}
